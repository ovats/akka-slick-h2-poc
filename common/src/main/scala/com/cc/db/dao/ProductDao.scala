...
object ProductDao extends BaseDao {
  ...
  def findById(id: ProductId): Future[Product]    = productsTable.filter(_.id === id).result.head
  def delete(idProd: ProductId): Future[Int]      = productsTable.filter(_.id === idProd).delete

  // Other operations
  def findByUUID(id: UUID): Future[Product]                  = productsTable.filter(_.uuid === id).result.head
